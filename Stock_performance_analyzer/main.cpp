...
double MACD(const int & current_index, const vector<pair<string, double>> & stock_data) {
  if (current_index < 26) return 0.0; // Ensure enough data points

  double short_EWM = stock_data[current_index - 12].second; 
  double long_EWM = stock_data[current_index - 26].second;
  double MACDi = 0.0;

  const int days_short = 12, days_long = 26;
  const double alpha_short = 2.0 / (days_short + 1);
  const double alpha_long = 2.0 / (days_long + 1);

  for (int i = current_index - 11; i <= current_index; i++) {
      short_EWM = alpha_short * (stock_data[i].second - short_EWM) + short_EWM;
  }

  for (int i = current_index - 25; i <= current_index; i++) {
      long_EWM = alpha_long * (stock_data[i].second - long_EWM) + long_EWM;
  }
...
int SIGNAL(const int &current_index,
  ...
  if (current_index < signal_period - 1)
  return 0;
    
  int start_index = current_index - signal_period + 1;
  double signal_line = MACD(start_index, stock_data);
    
  for (int i = start_index + 1; i <= current_index; i++) {
  signal_line = alpha * MACD(i, stock_data) + (1 - alpha) * signal_line;
  }
  
  double current_macd = MACD(current_index, stock_data);
  
  if (current_macd > signal_line)
  return 1;
  else if (current_macd < signal_line)
  return -1;
  else
  return 0;
}
